...
// 0번쨰 위치부터 2개를 제거하고 그 위치에 다른 요소들을 배치함
//제거한 수보다 추가되는 원소수가 많을 경우 배열이 확장되고 기존의 원소들은 뒤로 밀린다.
// 제어한 수보다 추가되는 원소수가 적을 경우 배열이 축소된다.
...
// arr.indexOf(item, from)은 인덱스 from부터 시작해 item(요소)를 찾는다.
// 요소를 발견하면 해당 요소의 인덱스를 반환한다.

// arr.lastIndexOf(item, from)은 위 메서드와 동일한 기능을 하는데 , 검색을 끝에서부터 시작한다.
// 두 번쨰 파라미터 (form)이 없으면 처음부터 탐색한다.
...
// indexOf 메서드는 요소를 찾을 때 완전 항등 연산자 ===를 사용한다는 점에 유의해야한다.
...
const arr2 = [NaN];
console.log(arr2.indexOf(NaN));  // -1   완전 항등 비교 ===은 NaN에는 동작하지 않으므로 0이 출력되지 않는다.
