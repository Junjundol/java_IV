...
// 단어나 문장으로 검색할 경우 일치하는 내뇽이 시작되는 첫 글자의 위치를 반환
console.log("'short'가 처음 나타나는 위치 : " 
 + msg.indexOf("short"));


// indexof 에 파라미터가 두 개인 경우,
// 두 번째 숫자 값은 첫 번째 파라미터의 글자를 찾기 시작하는 위치를 의미한다.
const p2 = msg.indexOf("i");
const p3 = msg.indexOf("i", p2+1);
console.log("'i가 첫 번재로 나타나는 위치 : " + p2);
console.log("'i가 두 번째로 나타나는 위치 : " + p3);


//파라미터로 저날된 글자가 마지막으로 나타나는 위치를 리턴한다.
...
// 두 번쨰 파라미터가 없을 경우 지정된 위치부터 끝까지 자르다.
...
// 첫 번쨰 파라미터의 내용을 두 번째 파라미터로 변경함 결과를 반환한다.
