...
// 주어진 판별함수를 만족하는 첫번째 값을 반환한다. 못찾으먄 undefined를 반환한다.
...
const found = arr6.find(function (v) {
    ...
    // ture를 리턴하는 순간 배열의 탐색을 중단한다.(break와 동일한 기능)
    if ( v % 2 == 0){
...
//배열의 각 요소를 순히하며 callback함수의 실행 값을 누적하여 하나의 결과값을 반환
let arr12 = [1,2,3,4,5];

/**
 * accumulator : 직전 콜백이 리턴한 값
 * currentValue : 이번 회차에 탐색되는 배열 원소
 *  index : 이번회차에 탐색되는 배열 원소의 인덱스(생략가능)
 ...
 * 
 * 두 번째 실행부터 accumulator에는 이전 회차에서 리턴한 값이 되돌아온다. 그리고 currentValue에는 2번째 부터 순서대로 매 실행회차마다 다음 원소가 전달 된다.
 * 
