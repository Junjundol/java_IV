...
class XlessError extends Error {
    //자식 클래스가 생성자를 갖을 경우 부모의 생성자를 반드시 강제호출 해야한다.. --> suoer(...)
    constructor(msg) {
        super(msg);
...
function foo(x,y) {
    if (x < 0) {
        // 함수안에서 에러를 강제를 발생시킬 경우, 이 함수를 호출하는 위치를 에러로 인식한다.
...
// 에러점검이 끝난 후 try-catch 블록 밖에서 k 값을 활용하려면
//변수의 선언 위치가try블록보다 상위에 취에해아한다. --> 변수의 스코프 규칙
const a= null;
const b= null;
